...
interface SkillHubScope {
  getSnapshot(): ScopeSnapshot
  subscribe(listener: () => void): () => void
  set(field: string, value: unknown): Promise<void>
}

interface SkillHubRpc {
  call(channel: string, endpoint: string, payload: unknown): Promise<any>
}

type SaveState = 'idle' | 'saving' | 'saved' | 'error'

function validationError(value: string): string | undefined {
  if (value.trim() === '') return undefined
  try {
    const url = new URL(value)
    if (url.protocol !== 'https:' || !['github.com', 'gitlab.com'].includes(url.hostname.toLowerCase())) {
      return '请输入 GitHub 或 GitLab 的 HTTPS 仓库地址。'
    }
  } catch {
    return '请输入有效的仓库地址。'
  }
...
function SkillHubSettings({ scope, rpc }: { scope: SkillHubScope; rpc: SkillHubRpc }): any {
  ...
  const canSave = draft.trim() !== '' && error === undefined && !disabled && saveState !== 'saving'

  useEffect(() => { setDraft(persisted) }, [persisted])
  useEffect(() => { if (saveState !== 'idle') setSaveState('idle') }, [draft])
  ...
  return createElement('section', { className: 'dsh-skill-hub-section' },
    createElement('h2', { className: 'dsh-skill-hub-title' }, 'Skill Hub'),
    createElement('p', { className: 'dsh-skill-hub-intro' }, '团队技能仓库将在 SessionStart 时同步。'),
    createElement('form', {
      className: 'dsh-skill-hub-form',
      onSubmit: (event: any) => { event.preventDefault(); void save() },
    },
    createElement('div', { className: 'dsh-skill-hub-field' },
      createElement('div', { className: 'dsh-skill-hub-field-head' },
        createElement('label', { className: 'dsh-skill-hub-label', htmlFor: 'skill-hub-repository-url' }, '仓库 URL'),
        dirty ? createElement('span', { className: 'dsh-skill-hub-badge' }, '未保存') : null,
      ),
      createElement('input', {
        className: error === undefined ? 'dsh-skill-hub-input' : 'dsh-skill-hub-input dsh-skill-hub-input-invalid',
        ...
        'aria-describedby': error === undefined ? 'skill-hub-repository-hint' : 'skill-hub-repository-error',
        onChange: (event: any) => { setDraft(event.currentTarget.value) },
      }),
      error === undefined
        ? createElement('p', { className: 'dsh-skill-hub-hint', id: 'skill-hub-repository-hint' }, '支持 GitHub 和 GitLab HTTPS 地址。')
        : createElement('p', { className: 'dsh-skill-hub-error', id: 'skill-hub-repository-error', role: 'alert' }, error),
    ),
    createElement('div', { className: 'dsh-skill-hub-footer' },
      createElement('p', { className: 'dsh-skill-hub-feedback', 'aria-live': 'polite' },
        saveState === 'saving' ? progress : saveState === 'saved' ? progress : saveState === 'error' ? progress : ''),
      createElement('button', {
        className: 'dsh-skill-hub-save', type: 'submit',
        disabled: !canSave,
...
export function apply(ctx: ClientContext): void {
  const rawScope = ctx.settingsScope.bind({ namespace: SETTINGS_NAMESPACE }) as unknown as SkillHubScope
  const rpc = (ctx as any).get('connection')?.rpc as SkillHubRpc
  const scope: SkillHubScope = {
  ...
  ctx.slots.inject('settings.section', () => ctx.slots.register({
    name: 'settings.section', id: 'skill-hub', order: 25, label: 'Skill Hub',
  }, () => createElement(SkillHubSettings, { scope, rpc })))
}
